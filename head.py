...
class RTMDetInsHead(RTMDetHead):
    ...
    def loss_mask_by_feat(self, mask_feats: Tensor, flatten_kernels: Tensor,
        ...
        #用于loss_mask为CrossEntropyLoss
        pos_gt_masks = pos_gt_masks.to(dtype=batch_pos_mask_logits.dtype)
...
from vis0316001 import draw_feature_map
@MODELS.register_module()
    ...
    def forward(self, feats: Tuple[Tensor, ...]) -> tuple:
        ...
        for idx, (x, stride) in enumerate(
            ...
            for cls_layer in self.cls_convs[idx]:
                cls_feat_1 = cls_layer(cls_feat_1)
            cls_feat = cls_feat + cls_feat_1
            ...
            for kernel_layer in self.kernel_convs[idx]:
                kernel_feat_1 = kernel_layer(kernel_feat_1)
            kernel_feat = kernel_feat + kernel_feat_1
            kernel_pred = self.rtm_kernel[idx](kernel_feat)

            for reg_layer in self.reg_convs[idx]:
                reg_feat_1 = reg_layer(reg_feat_1)
            reg_feat = reg_feat + reg_feat_1
